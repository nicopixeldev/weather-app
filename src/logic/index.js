...
const logic = {
    ...
    getWeatherByCityName(cityName) {
        if (typeof cityName !== 'string') throw TypeError(`${cityName} is not a string`)
        if (!cityName.trim().length) throw Error('city name can not be empty')
        
        return fetch(`${REACT_APP_API_HOST}/forecast/daily?key=${REACT_APP_API_KEY}&city=${cityName}&days=7`)
